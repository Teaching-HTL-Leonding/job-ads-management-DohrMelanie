...
export class JobManagementService {
  ...
  public deleteJob(jobId: number): Promise<void> {
    return firstValueFrom(
      this.httpClient.delete<void>(`${this.baseUrl}/ads/${jobId}`)
    );
  }

  public getJobById(jobId: number): Promise<Job> {
    return firstValueFrom(
      this.httpClient.get<Job>(`${this.baseUrl}/ads/${jobId}`)
    );
  }

  public updateJob(job: Job): Promise<void> {
    return firstValueFrom(
      this.httpClient.patch<void>(`${this.baseUrl}/ads/${job.id}`, job)
    );
  }
  ...
  public async addTranslation(
    ...
    return firstValueFrom(
      this.httpClient.put<void>(
        `${this.baseUrl}/ads/${jobId}/translations/${lang}`,
        {
  ...
  public async autoTranslate(jobId: number, lang: string) {
    ...
    const translated: TranslationResp = await firstValueFrom(
      this.httpClient.post<TranslationResp>(
        `${this.baseUrl}/deepl/v2/translate`,
        translate
      )
    );
